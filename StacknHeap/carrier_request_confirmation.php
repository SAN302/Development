...
<?php
	...
	{
		$declined=1;
		$conid=$_SESSION['confirmation_id'];
		$sql="UPDATE confirmation SET declined='$declined' WHERE id='$conid'";
		mysqli_query($conn,$sql);
		header("location: carrier_request_confirmation.php");
	}
...
<a class="navbar-brand">
	...
	<?php
	$buyerid=$_SESSION['id'];
	$sql="select con.id,su.fname,con.declined,con.payment from signup su,confirmation con where su.id=con.carrier_id AND buyerid='$buyerid'";
	...
	if($numrows>0)
	{
		...
		{	
			$_SESSION['confirmation_id']=$row['id'];
			if($row['declined']==0 && $row['payment']==0)
			{
				echo "".$row['fname']." has confirmed your request	";
				
				echo"<a href='payment.php'><input id='acceptbtn' class='btn' type='submit' data-toggle='modal' data-target='#myModal' name='accept_button' value='Pay' /></a>";
				echo"<form name='form' action='' method='POST'><input id='declinebtn' class='btn' type='submit' data-toggle='modal' data-target='#myModal' name='decline_button' value='Decline' /></a>";
				break;
			}
			elseif($numrows==0)
			{
				echo "<h1>No Confirmations Requests</h1>";
				//break;
			}
		}			
	}
	...
	{
		echo "No Requests Confirmed yet.";
	}
